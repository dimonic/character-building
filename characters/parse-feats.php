...
while ($line = fgets($FEATS, $max_length)) {
	...
	case 'h3':
	    ...
	    $query = "INSERT INTO `feats` (`name`, `type`, `Fighter`, `prerequisites`, `benefit`, `normal`, `special`, `source`) VALUES ('$feat[name]', '$feat[type]', '$fighter', '$feat[Prerequisites]', '$feat[Benefit]', '$feat[Normal]', '$feat[Special]', 'PHB')";
	    issue_query($query);
	    $feat = array();
	    $accumulate = "";
	    ereg("(.*)\[(.*)\]", $label, $part);
