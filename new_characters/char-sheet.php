...
//    $this->Rect($xoff + 76, $line, 9, 6);
  ...
  function ac_block($xoff, $line)
  {
    ...
    $this->box_value(max($character[armor_bonus] + $character[armor_magic], $armor), 8, $xoff + 97, $line + 3.5, 7, 6);
    $this->text_l("+", 6, $xoff + 103.5, $line +6.5);
    $this->label_2("SHIELD BONUS", $xoff + 105, $line + 1.5);
    $this->box_value(max($character[shield_bonus] + $character[shield_magic], $shield), 8, $xoff + 106, $line + 3.5, 7, 6);
    ...
    if (!empty($character["armor_max_dex"]) || !empty($character["shield_max_dex"]))
      ...
      $max_dex = 99;
    $this->box_value(min($max_dex, att_bonus('dex')), 8, $xoff + 115, $line + 3.5, 7, 6);
    ...
    $this->box_value(($size == 'Small' ? 1 : $size == 'Large' -1), 8, $xoff + 133, $line + 3.5, 7, 6);
  ...
  function skill_list($xoff, $line)
  {
	  ...
	  $total_bonus = bonus_total($bonus_by_name[$name]);
  ...
  function special_abilities($n, $xoff, $line)
  {
	...
	// first we check for a number in the special
        if (preg_match('/^(.*\+)([0-9]+)(.*)$/', $separate_ability[$c][$d], $this_parts)) {
